...
class RtcpIterator(buf: ByteBuffer) {
    ...
    fun next(): ReadOnlyRtcpPacket {
        if (!hasNext()) {
            throw Exception("No more items left on iterator")
        }
        val subBuf = buf.slice()
        try {
            val packet = ReadOnlyRtcpPacket.fromBuffer(subBuf)
    ...
    fun getAll(): List<ReadOnlyRtcpPacket> {
        val packets = mutableListOf<ReadOnlyRtcpPacket>()
        while (hasNext()) {
            packets.add(next())
        }
        return packets
