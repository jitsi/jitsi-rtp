...
class RtcpIterator(buf: ByteBuffer) {
    ...
    fun next(): ImmutableRtcpPacket {
        if (!hasNext()) {
            throw Exception("No more items left on iterator")
        }
        val subBuf = buf.slice()
        try {
            val packet = ImmutableRtcpPacket.fromBuffer(subBuf)
    ...
    fun getAll(): List<ImmutableRtcpPacket> {
        val packets = mutableListOf<ImmutableRtcpPacket>()
        while (hasNext()) {
            packets.add(next())
        }
        return packets
